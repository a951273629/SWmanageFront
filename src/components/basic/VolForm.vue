			...
			<div class="vol-form-item">
				...
				>
					...
					<span
						v-else-if="
							item.readonly &&
								typeof formFields[item.field] == 'function'
						"
					...
					<div v-else-if="isReadonlyImgFile(item, formFields)">
						<div v-if="item.type == 'img'" class="form-imgs">
							<div
								class="img-item"
								v-for="(img, imgIndex) in formFields[
									item.field
								]"
								:key="imgIndex"
							>
					...
					<div v-else :class="{ 'form-item-extra': item.extra }">
						...
						<template
							v-else-if="
								['select', 'selectList'].indexOf(item.type) !=
									-1
							"
						>
							<el-select-v2
						...
						<div
							...
							style="display: flex"
							v-else-if="
								['date', 'datetime'].indexOf(item.type) != -1 &&
									item.range
							"
						>
							<el-date-picker
								...
								:type="
									item.type == 'date' ? 'date' : 'datetime'
								"
								:disabledDate="
									(val) => getDateOptions(val, item)
								"
								placeholder="开始时间"
								@change="
									(val) => {
							...
							<span
								style="margin: 0px 5px; font-size: 13px; color: #6f6b6b"
								>至</span
							>
							<el-date-picker
								:size="size"
						...
						<vol-upload
							...
							:url="item.url"
							:img="
								item.type == 'img' || item.columnType == 'img'
							"
						...
						<el-cascader
							...
							:props="{
								checkStrictly:
									item.changeOnSelect || item.checkStrictly
							}"
							@change="item.onChange"
						>
						</el-cascader>
						...
						<div class="form-extra" v-if="item.extra">
							...
							>
								<i
									v-if="item.extra.icon"
									:class="item.extra.icon"
								/>
								{{ item.extra.text }}
							</a>
	...
	export default defineComponent({
		...
		computed: {
			rules() {
				...
				this.formRules.forEach((option, xIndex) => {
					option.forEach((item) => {
						ruleResult[item.field] = [
							this.getRule(item, this.formFields)
						];
					});
				});
				...
				props.formRules.forEach((row, xIndex) => {
					...
					row.forEach((item, yIndex) => {
						...
						if (item.range || item.type == "range") {
							if (
								!(
									props.formFields[item.field] instanceof
									Array
								) ||
			...
			this.formRules.forEach((rules) => {
				rules.forEach((option) => {
					...
					if (
						...
						option.dataKey === "编码器级联规则"
					) {
						option.onChange = async (option) => {
							if(!TemptThis.formFields.ParentID){
								return;
							}
							let last = TemptThis.formFields.ParentID.length - 1;
							console.log(JSON.stringify(TemptThis.formFields));
							//  自己的ID和父ID相同
							let same = TemptThis.formFields.ParentID.find(
								function(parentId) {
									return (
										parentId ===
										Number(TemptThis.formFields.ID)
									);
								}
							);
							// 选择了自己的子级作为父级提示错误。重新选择
							if (same) {
								TemptThis.formFields.ParentID = [];
								ElMessage.error(
									"请选择其他目录，不能将自己或自己的子级，设置为父目录"
								);
								return;
							}
							...
							let result = await http.post(
								...
								{ id: TemptThis.formFields.ParentID[last] }
							);
							//  1说明是一个目录可以选择
							if (result.ruleTypeCode === "1") {
								return;
								// 2说明是一个物料不能当做目录使用
							} else {
								// 删除数组的最后一位 调用splice方法vue才能监听到
								TemptThis.formFields.ParentID = TemptThis.formFields.ParentID.splice(
									0,
									last
								);
								ElMessage.error(
									"当前选中的层级不是一个物料目录，请选择一个目录"
								);
							}
							//  TemptThis.formFields.ParentID=[];
						};
		...
		methods: {
			getColWidth(item) {
				...
				let rete =
					Math.round(
						((item.colSize || 12 / _span) / 0.12) * colPow,
						10.0
					) / colPow;
			...
			getRule(item, formFields) {
				...
				if (item.validator && typeof item.validator === "function") {
					return {
						...
						trigger:
							rule.change.indexOf(item.type) != -1
								? "change"
								: "blur"
					};
				}
					...
					return {
						...
						validator: (ruleObj, value, callback) => {
							...
							if (
								...
								ruleObj.message =
									ruleObj.title + "不能小于" + ruleObj.min;
								return callback(new Error(ruleObj.message));
							}
							if (
								...
								ruleObj.message =
									ruleObj.title + "不能大于" + ruleObj.max;
								return callback(new Error(ruleObj.message));
							}
							return callback();
						}
				...
				if (inputTypeArr.indexOf(item.type) != -1) {
					let message =
						...
						(item.type == "mail"
							...
							: "不能为空");
					let type =
						item.type == "mail" ? "email" : types[item.columnType];
					let _rule = {
						required: item.required,
						message: message,
						trigger: "blur",
			...
			getDateOptions(date, item) {
				...
				return (
					this.compareDate(date, item.min) ||
					!this.compareDate(date, item.max)
				);
			},
			getDateFormat(item) {
				if (item.type == "month") {
...
<style lang="less" scoped>
	...
	.vol-form-item
		::v-deep(.el-input-number--large .el-input-number__increase) {
		border-top: 1px solid #d4d4d4;
	}
	.vol-form-item
		::v-deep(.el-input-number--large .el-input-number__decrease) {
